...
$sqlIngreso = "SELECT SUM(Monto) AS ingreso_total FROM ingresos WHERE YEAR(Fecha) = $anio AND MONTH(Fecha) = $mes";
$resultIngreso = $conn->query($sqlIngreso);
$ingresoTotal = $resultIngreso->fetch_assoc()['ingreso_total'] ?? 0;

$sqlGasto = "SELECT SUM(Monto) AS gasto_total FROM egresos WHERE YEAR(Fecha) = $anio AND MONTH(Fecha) = $mes";
...
$sqlBalanceAnterior = "SELECT 
    (COALESCE((SELECT SUM(Monto) FROM ingresos WHERE YEAR(Fecha) = $anioAnterior AND MONTH(Fecha) = $mesAnterior), 0) 
    - 
    COALESCE((SELECT SUM(Monto) FROM egresos WHERE YEAR(Fecha) = $anioAnterior AND MONTH(Fecha) = $mesAnterior), 0)
    ) AS balance_anterior";
...
$sqlCantidadIngresos = "SELECT COUNT(*) AS cantidad FROM ingresos WHERE YEAR(Fecha) = $anio AND MONTH(Fecha) = $mes";
$resultCantidadIngresos = $conn->query($sqlCantidadIngresos);
$cantidadIngresos = $resultCantidadIngresos->fetch_assoc()['cantidad'] ?? 0;

$sqlCantidadGastos = "SELECT COUNT(*) AS cantidad FROM egresos WHERE YEAR(Fecha) = $anio AND MONTH(Fecha) = $mes";
$resultCantidadGastos = $conn->query($sqlCantidadGastos);
$cantidadGastos = $resultCantidadGastos->fetch_assoc()['cantidad'] ?? 0;

$sqlIngresoAnual = "SELECT SUM(Monto) AS ingreso_anual FROM ingresos WHERE YEAR(Fecha) = $anio";
$resultIngresoAnual = $conn->query($sqlIngresoAnual);
$IngresoAnual = $resultIngresoAnual->fetch_assoc()['ingreso_anual'] ?? 0;

$sqlGastoAnual = "SELECT SUM(Monto) AS gasto_anual FROM egresos WHERE YEAR(Fecha) = $anio";
